...
- Open [[./doc/Week%201%20Silver Task.docx][this worksheet]] and then Watch the following three videos and decide which (if any) of the goals listed above have been achieved by the AIs shown by putting ticks in the table. For example, put tick in the =reasoning= column if the AI can perform reasoning in table of the open document.
- Answer the other two questions in the open document and save to your homedrive and upload to BTL
#+BEGIN_EXPORT html
